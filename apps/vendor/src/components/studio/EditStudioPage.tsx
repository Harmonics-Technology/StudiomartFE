...
export const EditStudioPage = ({ singleStudio }: StudioProps) => {
  const {
    handleSubmit,
    register,
    watch,
    ...
    defaultValues: {
      ...
      country: singleStudio.country,
      coverPhoto: singleStudio.coverPhoto,
      description: singleStudio.description,
      email: singleStudio.email,
      facebook: singleStudio.facebook,
      id: singleStudio.id,
      instagram: singleStudio.instagram,
      linkedIn: singleStudio.linkedIn,
      logo: singleStudio.logo,
      name: singleStudio.name,
      phone: singleStudio.phone,
      state: singleStudio.state,
      twitter: singleStudio.twitter,
      website: singleStudio.website,
      youTube: singleStudio.youTube,
      zipCode: singleStudio.zipCode,
    },
  });
  ...
  const onSubmit = async (data: StudioModel) => {
    ...
    logoUrl ? (data.logo = logoUrl) : (data.logo = data.logo);
    try {
      const result = await StudioService.updateStudio({ requestBody: data });
      if (result.status) {
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <Box mt="1.5rem">
            ...
            >
              {bannerUrl || singleStudio.coverPhoto ? (
                <Box
                  role="group"
                  pos="relative"
                  w="full"
                ...
                >
                  <Image
                    src={bannerUrl || (singleStudio.coverPhoto as string)}
                    alt="Banner Image"
                    w="full"
                    h="full"
                    objectFit="cover"
